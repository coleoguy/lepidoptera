...
# load results
load("../results/5a.false.positive.test.Lepidoptera.rate.analysis.feeding.RData")
results <- x
...
poorMCMC <- c(90,42,39,34,30,20,11)
# see when the mcmc have converged
plot(results[[1]]$p, type = "l",
     ylim = c(-1700,-1450),
     col = rainbow(1,alpha = .5))
...
# write poorMCMC results
write.csv(x = poorMCMC, file = "../results/5a.fase.postivie.poorMCMC.csv",row.names = F)
...
write.csv(x = post.burnin,
          file = "../results/5a.false.positive.test.Lepidoptera.rate.analysis.feeding.csv",
          row.names = F)
