...
# load data
load("simulate.data.rr-10.RData")
...
results.condition.6 <- vector(mode = "list", length = 5)
...
names(results.condition.6) <- c(2,4,6,8,10)
...
for(i in 1:5){
  x <- foreach(j = 1:100, .verbose = T, .packages = c("ape","diversitree", "chromePlus")) %dopar% {
    # make the initial data frame
    chroms <- NULL
    MCMC.dat <-  data.frame(species = names(chrom.traits.6[[i]][[j]]),
                            Chroms = chrom.traits.6[[i]][[j]],
                            bin = bin.traits.6[[i]][[j]],
                            stringsAsFactors = F,
                            row.names = NULL)
    # get the range of chromosome number
    rng <- NULL
...
stopCluster(cl)
# save
save.image("mcmc.simulate.data.rr-10.RData")
