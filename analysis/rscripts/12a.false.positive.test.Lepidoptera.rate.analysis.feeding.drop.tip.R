...
# read data
dat <- read.csv("../results/11.Lepidoptera.rate.analysis.feeding.drop.tip.csv")
...
x <- foreach(j = 1:100, .verbose = T, .packages = c("ape","maps","diversitree", "phytools", "chromePlus")) %dopar% {
  ...
  chrom.sorted <- chrom.sorted[!commonTips$x,]
...
stopCluster(cl)
# save
save.image("../results/12a.false.positive.test.Lepidoptera.rate.analysis.feeding.drop.tip.RData")
