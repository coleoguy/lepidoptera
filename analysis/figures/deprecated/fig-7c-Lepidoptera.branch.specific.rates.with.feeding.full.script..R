...
# set k
k <- sample(1:length(trees),size = 1)
...
parMat[!(parMat %in% c(1,2,3,4,5,6,8,9))] <- 0
...
# fill the qmat
parMat[parMat == 1] <- mean(dat$asc1)
parMat[parMat == 2] <- mean(dat$desc1)
parMat[parMat == 3] <- mean(dat$asc2)
parMat[parMat == 4] <- mean(dat$desc2)
parMat[parMat == 5] <- mean(dat$pol1)
parMat[parMat == 6] <- mean(dat$pol2)
parMat[parMat == 8] <- mean(dat$tran12)
parMat[parMat == 9] <- mean(dat$tran21)
...
for(i in 1:Ntip(tree)){
        dat.tips$tip[i] <- i
        dat.tips$chrom[i] <- MCMC.dat$Chroms[MCMC.dat$Species == tree$tip.label[i]]
}
...
radi <- getRadius(scale = 0.5,
                  ...
                  tree = tree,
                  tip.labels = FALSE,
                  trait.values = MCMC.dat$tipRates,
                  classes = 5)
...
ggplot(MCMC.dat, aes(x = log(tipRates), 
                     fill = as.factor(gen.prob))) +
...
ggplot(MCMC.dat, aes(x = log(tipRates + 1), 
                     fill = as.factor(gen.prob))) +
        geom_histogram(position = "dodge2",
                       binwidth = 0.25) +
        scale_fill_discrete("Larval feeding type",
...
ggplot(MCMC.dat, aes(x = sqrt(tipRates), 
                     fill = as.factor(gen.prob))) +
        geom_histogram(position = "dodge2",
                       binwidth = 0.25) +
        scale_fill_discrete("Larval feeding type",
...
#         
#         # top circle limit
#         theta <- atan(obj$yy[which.max(MCMC.dat$tipRates)]/obj$xx[which.max(MCMC.dat$tipRates)])
#         if(obj$xx[which.max(MCMC.dat$tipRates)] > 0){
#                 s <- 1
#         }else{
#                 s <- -1
#         }
...
#         x3 <- s * x[which.max(MCMC.dat$tipRates)] * cos(theta) + x2
#         y3 <- s * x[which.max(MCMC.dat$tipRates)] * sin(theta) + y2
#         x4 <- s * x[which.max(MCMC.dat$tipRates)] * cos(theta) + x1
#         y4 <- s * x[which.max(MCMC.dat$tipRates)] * sin(theta) + y1
#         
#         x3.mean <- s * x[which.max(MCMC.dat$tipRates)]/2 * cos(theta) + x2
#         y3.mean <- s * x[which.max(MCMC.dat$tipRates)]/2 * sin(theta) + y2
#         x4.mean <- s * x[which.max(MCMC.dat$tipRates)]/2 * cos(theta) + x1
#         y4.mean <- s * x[which.max(MCMC.dat$tipRates)]/2 * sin(theta) + y1
#         
#         
#         xMaxTopCircle <- max(abs(c(x1, x2, x3, x4))) 
#         yMaxTopCircle <- max(abs(c(y1, y2, y3, y4)))
#         topCircle <- sqrt(xMaxTopCircle^2 + yMaxTopCircle^2) 
#         
#         xMaxMiddleCircle <- max(abs(c(x1, x2, x3.mean, x4.mean))) 
#         yMaxMiddleCircle <- max(abs(c(y1, y2, y3.mean, y4.mean)))
#         MiddleCircle <- sqrt(xMaxMiddleCircle^2 + yMaxMiddleCircle^2) 
#         
#         labs <- round(max(MCMC.dat$tipRates),0)
#         draw.circle(x = 0,
#                     y = 0,
#                     radius = c(topCircle,
#                                MiddleCircle),
...
# percentile <- 0.95
# specialists <- MCMC.dat$tipRates[MCMC.dat$gen.prob == 0 & MCMC.dat$tipRates > quantile(MCMC.dat$tipRates, percentile)]
# generalists <- MCMC.dat$tipRates[MCMC.dat$gen.prob == 1 & MCMC.dat$tipRates > quantile(MCMC.dat$tipRates, percentile)]
# 
# specialistRates <- density(specialists, bw = 1)
# generalistRates <- density(generalists, bw = 1)
# # get the HPD
# specialistHPD <- HPDinterval(as.mcmc(specialists))
# generalistHPD <- HPDinterval(as.mcmc(generalists))
...
#                   generalistRates$y)),
#      xlab = expression(paste("Tip rate ", "(MY"^-1,")")),
#      ylab = "Density")
# # Specialist rates
