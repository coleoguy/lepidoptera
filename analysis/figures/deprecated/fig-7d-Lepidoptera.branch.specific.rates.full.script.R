...
# make breaks
obj <- get("last_plot.phylo", envir = .PlotPhyloEnv)
theta <- atan(obj$yy[1]/obj$xx[1])
dx <- scale * max(nodeHeights(tree)) * cos(theta) + scale * cos(theta) * strwidth("l")
dif <- dx - max(nodeHeights(tree))
top <- max(MCMC.dat$tipRates) + dif
labs <- round(max(MCMC.dat$tipRates),0)
draw.circle(x = 0,
            y = 0,
            radius = c(top + max(branching.times(tree)),
                       top - (top / 4) + max(branching.times(tree)),
                       top / 2 + max(branching.times(tree)),
                       top / 4 + max(branching.times(tree))),
            nv=100,
            border="gray",
            col=NA,
            lty=2,
...
text(x = c(top + max(branching.times(tree)),
           top - (top / 4) + max(branching.times(tree)),
           top / 2 + max(branching.times(tree)),
           top / 4 + max(branching.times(tree))),
     y = rep(0, 4),
