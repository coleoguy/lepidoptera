...
tree <- read.nexus('heliconiini-MCC.tre')
dd <- read.table('heliconiini-lability.csv')

hosts <- dd$H
names(hosts) <- rownames(dd)

td <- treedata(tree, hosts)
t <- td$phy
...
lik.102.L <- diversitree::constrain(lik.102, qH01.L  ~ qH01.0*qH10.L/qH10.0, qL01.H ~0, qL10.H ~0)
...
lik.102.L0 <- diversitree::constrain(lik.102.L, lambdaL ~ 0)
